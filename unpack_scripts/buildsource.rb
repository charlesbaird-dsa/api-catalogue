...
def load_catalogue(tx, path)
  ...
  File.open "source/dashboard/index.html.md.erb", 'w' do |file|
  ...
  file.write  "\n<div style=\"height:1px;font-size:1px;\">&nbsp;</div>\n\n"
  org_count.each { |org| 
    file.write  "|#{org['organisation']}|#{org['number']}|#{org['firstadded']}|#{org['mostrecent']}|[#{org['provider']}](/api-catalogue/#{org['provider']}/)|\n"
   }
